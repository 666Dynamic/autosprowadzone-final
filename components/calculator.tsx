...
export function Calculator() {
  ...
  const [exchangeRate, setExchangeRate] = useState<number>(
    CALCULATOR_CONFIG.DEFAULT_EXCHANGE_RATE
  );
  ...
  const exciseRate = isLargeEngine
    ? CALCULATOR_CONFIG.AKC_RATE_HIGH
    : CALCULATOR_CONFIG.AKC_RATE_LOW;
  const excise = pricePln * exciseRate;
  const inspection =
    CALCULATOR_CONFIG.INSPECTION_BASE +
    (isLpg ? CALCULATOR_CONFIG.INSPECTION_LPG : 0) +
    (isAccident ? CALCULATOR_CONFIG.INSPECTION_ACCIDENT : 0);
  const registration = isIndividualPlates
    ? CALCULATOR_CONFIG.REGISTRATION_INDIVIDUAL
    : CALCULATOR_CONFIG.REGISTRATION_STANDARD;
  const totalCost =
    pricePln +
    excise +
    inspection +
    CALCULATOR_CONFIG.TRANSLATION +
    registration +
    CALCULATOR_CONFIG.COMMISSION;
      ...
      <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 items-stretch relative z-10 max-w-full">
          ...
          <CardContent className="flex-1 p-6 md:p-8 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label
                  htmlFor="price"
                  className="text-xs font-bold uppercase tracking-[0.2em] text-primary/80 ml-1"
                >
                  Cena (EUR)
                </Label>
                <div className="relative group">
                  ...
                  />
                  <div
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-primary font-black text-base"
                    aria-hidden="true"
                  >
                    €
                  </div>
              ...
              <div className="space-y-2">
                <Label
                  htmlFor="rate"
                  className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground ml-1"
                >
                  Kurs EUR (PLN)
                </Label>
                <div className="relative group">
                  ...
                  />
                  <div
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/50 font-bold text-sm"
                    aria-hidden="true"
                  >
                    zł
                  </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-2">
                {[
                  {
                    id: "engine",
                    checked: isLargeEngine,
                    set: setIsLargeEngine,
                    label: "Silnik powyżej 2.0L",
                    sub: "Akcyza 18.6%",
                  },
                  {
                    id: "lpg",
                  ...
                  {
                    id: "plates",
                    checked: isIndividualPlates,
                    set: setIsIndividualPlates,
                    label: "Tablice Indywidualne",
                    sub: "+918 PLN",
                  },
                ].map((item) => (
                  <div
                    ...
                    className={`group flex items-center space-x-3 p-3.5 rounded-xl border transition-all cursor-pointer select-none touch-manipulation min-h-[52px] focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 outline-none ${
                      item.checked
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`font-bold tracking-tight transition-colors ${item.checked ? "text-primary dark:text-primary" : "text-foreground group-hover:text-primary"}`}
                      >
                        {item.label}
                      </p>
                      <p className="text-xs uppercase font-bold tracking-widest text-muted-foreground group-hover:text-primary/70 transition-colors">
                        {item.sub}
                      </p>
                    </div>
                  </div>
        ...
        <Card className="flex flex-col border-2 border-primary/20 shadow-lg rounded-2xl overflow-hidden bg-card/50 backdrop-blur-sm text-card-foreground transition-all duration-300 hover:border-primary/30 hover:shadow-xl">
          <CardHeader className="p-6 md:p-8 border-b border-border bg-muted/30 min-h-[100px] flex flex-col justify-center">
            <CardTitle className="text-xl md:text-2xl font-black uppercase tracking-tighter">
              Wstępna Wycena
            </CardTitle>
            <CardDescription className="text-muted-foreground font-bold uppercase tracking-[0.2em] text-xs mt-2">
              Bez kosztów transportu (wycena indywidualna).
            </CardDescription>
          </CardHeader>
          <CardContent className="flex-1 p-6 md:p-8 space-y-5">
            ...
            <div className="bg-gradient-to-br from-background to-muted border-2 border-primary/20 rounded-2xl p-6 md:p-8 flex flex-col items-center justify-center text-center space-y-2 relative overflow-hidden group shadow-lg shadow-primary/5">
              ...
              <span className="text-[10px] md:text-xs text-muted-foreground uppercase font-black tracking-wider md:tracking-[0.3em] relative z-10">
                Cena Całkowita w Kraju
              </span>
              ...
              <Table>
                <TableBody>
                  <TableRow className="hover:bg-muted/50 transition-colors border-none group">
                    <TableCell className="text-[10px] md:text-sm font-bold py-3 text-muted-foreground group-hover:text-foreground uppercase tracking-wider transition-colors">
                      Cena Auta ({eurPrice} EUR)
                    </TableCell>
                    <TableCell className="text-right text-sm md:text-base font-black text-foreground py-3">
                      {formatCurrency(pricePln)}
                    </TableCell>
                  </TableRow>
                  <TableRow className="hover:bg-muted/50 transition-colors border-none group">
                    <TableCell className="text-[10px] md:text-sm font-bold py-3 text-muted-foreground group-hover:text-foreground uppercase tracking-wider transition-colors">
                      Podatek Akcyzowy ({isLargeEngine ? "18.6%" : "3.1%"})
                    </TableCell>
                    <TableCell className="text-right text-sm md:text-base font-black text-foreground py-3">
                      {formatCurrency(excise)}
                    </TableCell>
                  </TableRow>
                  <TableRow className="hover:bg-muted/50 transition-colors border-none group">
                    <TableCell className="text-[10px] md:text-sm font-bold py-3 text-muted-foreground group-hover:text-foreground uppercase tracking-wider transition-colors">
                    ...
                    <TableCell className="text-right text-sm md:text-base font-black text-foreground py-3">
                      {formatCurrency(inspection)}
                    </TableCell>
                  </TableRow>
                  <TableRow className="hover:bg-muted/50 transition-colors border-none group">
                    <TableCell className="text-[10px] md:text-sm font-bold py-3 text-muted-foreground group-hover:text-foreground uppercase tracking-wider transition-colors">
                      Tłumaczenia i rejestracja
                    </TableCell>
                    <TableCell className="text-right text-sm md:text-base font-black text-foreground py-3">
                      {formatCurrency(
                        CALCULATOR_CONFIG.TRANSLATION + registration
                      )}
                    </TableCell>
                  </TableRow>
                  {/* Commission - Toned Down */}
                  <TableRow className="border-t border-border/50">
                    <TableCell className="text-[10px] md:text-sm font-bold py-4 text-primary uppercase tracking-wider">
                      Prowizja serwisu
                    </TableCell>
                    <TableCell className="text-right py-4 font-black text-lg text-primary">
                      {formatCurrency(CALCULATOR_CONFIG.COMMISSION)}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
        ...
        <div className="col-span-full mt-8 space-y-6">
          <div className="bg-muted/50 border border-border rounded-xl p-6 text-center space-y-2 relative overflow-hidden group hover:border-primary/20 transition-all max-w-2xl mx-auto">
            <p className="text-xs font-bold text-primary uppercase tracking-wider md:tracking-[0.3em] mb-1">
              Logistyka i Transport
            </p>
            <p className="text-sm text-muted-foreground leading-relaxed italic max-w-sm mx-auto">
              Transport wyceniamy indywidualnie w zależności od odległości.
              Koszt zazwyczaj mieści się w granicach{" "}
              <span className="text-foreground font-bold">1500 - 3500 zł</span>.
            </p>
          </div>

          <div className="max-w-2xl mx-auto space-y-4">
            <Link href="/#zlecenie">
              <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/90 transition-all h-11 font-bold rounded-lg shadow-lg">
