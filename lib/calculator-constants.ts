...
// Optimization: Cache Intl.NumberFormat instances globally to avoid expensive object recreation on every format call
...
export function formatCurrency(value: number, currency: 'PLN' | 'EUR' = 'PLN'): string {
  return currency === 'PLN' ? currencyFormatterPLN.format(value) : currencyFormatterEUR.format(value)
}
