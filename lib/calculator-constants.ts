...
// Cache Intl.NumberFormat instances to avoid performance overhead during frequent calls
...
export function formatCurrency(value: number, currency: 'PLN' | 'EUR' = 'PLN'): string {
  return currency === 'PLN'
    ? currencyFormatterPLN.format(value)
    : currencyFormatterEUR.format(value);
}
