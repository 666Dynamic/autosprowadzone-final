...
// Cached Intl.NumberFormat instances to prevent expensive re-instantiation
const CURRENCY_FORMATTER_PLN = new Intl.NumberFormat('pl-PL', {
  style: 'currency',
  currency: 'PLN',
...
const CURRENCY_FORMATTER_EUR = new Intl.NumberFormat('pl-PL', {
  style: 'currency',
  currency: 'EUR',
  minimumFractionDigits: 2,
  maximumFractionDigits: 2,
...
export function formatCurrency(value: number, currency: 'PLN' | 'EUR' = 'PLN'): string {
  return currency === 'PLN'
    ? CURRENCY_FORMATTER_PLN.format(value)
    : CURRENCY_FORMATTER_EUR.format(value)
}
